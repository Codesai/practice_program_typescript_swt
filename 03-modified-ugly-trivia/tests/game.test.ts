...
describe('Ugly trivia', () => {
    ...
    it('simulation with 5 players', () => {
        ...
        const game = createGameForTesting(rollNumbers, isWrongAnswers);
        game.add("Chet");
        game.add("Pat");
        game.add("Sue");
        game.add("Peter");
        ...
        checkNotifications(getExpectedLogForSimulationWith5Players());
    });
    ...
    it('simulation with 3 players', () => {
        ...
        checkNotifications(getExpectedLogForSimulationWith3Players());
    });
    ...
    it('simulation with 7 players', () => {
        ...
        const answersAreWrong: Array<boolean> = [false, false, false, false, false,
            ...
            false];
        const game = createGameForTesting(rollNumbers, answersAreWrong);
        ...
        checkNotifications(getExpectedLogForSimulationWith7Players());
    });
    ...
    function createGameForTesting(rollNumbers: Array<number>, answersAreWrong: Array<boolean>) {
        notifier = {
            notify: jest.fn()
        };
        return new GameForTesting(rollNumbers, answersAreWrong, notifier);
    }

    function checkNotifications(expectedNotifications: string[]): void {
        expect(notifier.notify.mock.calls).toEqual(expectedNotifications.map(message => [message]));
    }
});
...
class GameForTesting extends Game {
    ...
    constructor(rollNumbers: number[], isWrongAnswers: boolean[], notifier: Notifier) {
        super(notifier);
